...
class PlayblastManager(object):
    ...
    def doPlayblast(self, **overridePlayblastParams):
        ...
        if 'Shot' in self._context.entity.get('type'):
            template_in = self._app.get_template("template_shot_work")
            template_out = self._app.get_template("template_shot")
        else:
            template_in = self._app.get_template("template_asset_work")
            template_out = self._app.get_template("template_asset")            
        
        doc = c4d.documents.GetActiveDocument()
        sceneName = doc[c4d.DOCUMENT_FILEPATH]

        fields = template_in.get_fields(sceneName)
        self.shotPlayblastPath = template_out.apply_fields(fields)
    ...
    def __createPlayblast(self, **overridePlayblastParams):
        ...
        if self.__uploadToShotgun:
            ...
            data = { 'project': project,
                    ...
                    'sg_first_frame': doc[c4d.DOCUMENT_LOOPMINTIME].GetFrame(fps),
                    'entity': entity,
                    'sg_task': task,
                }
